...
class Database {
	...
	store = async (fPath, m) => {
		try{ 
			if(!fPath) return console.log("Provide a file path to the store folder")
			const stFile = path.join(fPath)
			
			const jsonData = JSON.stringify(m, null, 2);
			const arr = []
			const dataArr = arr.push(jsonData)
			fs.writeFileSync(stFile, dataArr, 'utf8');
		} catch (err) {
	...
	main = async (m) => {
		...
		if (m.isGroup) {
			const existingGroup = this.get('groups', m.from)
			if (!existingGroup || typeof existingGroup === 'string') {
				await this.addGroup('groups', m.from, {
					enable: true,
					welcome: false,   
					chatbot: false
				})
